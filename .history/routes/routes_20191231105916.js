...
pgrtrs.get('/blogs', async function(request, response){
   blogs = await bloglist.find().sort({"date": -1,"_id": -1}).limit(parseInt(request.params.limit))
  ...
  try{
    if(searchq == ""){
      response.redirect('/search/404');
         ...
         blogs = await bloglist.find().or([{title: {$regex: searchq, $options: "i"}}, {category: {$regex: searchq, $options: "i"}}, {tags: {$regex: searchq, $options: "i"}}, {blog_cont: {$regex: searchq, $options: "i"}}]).sort({"_id": -1}).limit(2);
