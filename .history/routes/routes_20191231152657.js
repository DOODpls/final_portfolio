...
pgrtrs.get("/get-post/:start/:limit", async function (request, response){
  ...
  if ( url == '/search/?q='+searchq ){
    ...
    blogs = await bloglistfind().or([{category: {$regex: searchq, $options: "i"}}]).sort({"_id": -1}).skip(parseInt(request.params.start)).limit(parseInt(request.params.limit));
}
  const categories = await bloglist.distinct('category');
    response.send({
      pginfo: pagesInfo.index,
  ...
  try{
    if(searchq == ""){
      response.redirect('/search/404');
        ...
        const blogs = await bloglist.where('tags', request.params.items).sort({"_id": -1}).limit(2);
        response.render('blogsearch',{
        blogres: blogs,
        query: searchq,
        pagettl: title,
  ...
  try{
    if(searchq == ""){
      response.redirect('/search/404');
        ...
        const blogs = await bloglist.where('category', request.params.items).sort({"_id": -1}).limit(2);
        response.render('blogsearch',{
        blogres: blogs,
        query: searchq,
        pagettl: title,
