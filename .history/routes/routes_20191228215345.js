...
pgrtrs.get('/blogs', async function(request, response){
  const blogs = await bloglist.find().sort({"_id": -1}).limit(2);
  bloglist.distinct('category', function(err, results){
...
pgrtrs.get("/get-posts/:start/:limit", async function (request, response){
  const blogs = await bloglist.find().sort({"_id": -1}).skip(parseInt(request.params.start)).limit(parseInt(request.params.limit)).toArray(
    function(err, posts){
      response.send(posts);
    }
  )
  ...
  try{
    if(searchq == ""){
      response.redirect('/search/404');
        ...
        const blogsearch = await bloglist.find().or([{title: {$regex: searchq, $options: "i"}}, {category: {$regex: searchq, $options: "i"}}, {tags: {$regex: searchq, $options: "i"}}, {blog_cont: {$regex: searchq, $options: "i"}}])
