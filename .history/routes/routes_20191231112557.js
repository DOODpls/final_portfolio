...
pgrtrs.get("/get-post/:start/:limit", async function (request, response){
  const blogs = await conf.sort({"date": -1,"_id": -1}).skip(parseInt(request.params.start)).limit(parseInt(request.params.limit))
  const categories = await bloglist.distinct('category');
    response.send({
      pginfo: pagesInfo.index,
      blogss: blogs,
...
pgrtrs.get('/search', async function(request, response){
  ...
  conf = 'bloglist.find().or([{title: {$regex: searchq, $options: "i"}}, {category: {$regex: searchq, $options: "i"}}, {tags: {$regex: searchq, $options: "i"}}, {blog_cont: {$regex: searchq, $options: "i"}}])'
  try{
    if(searchq == ""){
