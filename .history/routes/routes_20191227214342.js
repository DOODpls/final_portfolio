...
pgrtrs.get('/search', async function(request, response){
  ...
  try{
    ...
    const blogsearch = await bloglist.find().or([{title: {$regex: "", $options: "i"}}, {category: {$regex: "", $options: "i"}}, {tags: {$regex: "", $options: "i"}}, {blog_cont: {$regex: "", $options: "i"}}])
    response.redirect('blogsearch',{
    blogres: blogsearch,
    query: "",
    pginfo: pagesInfo.index
