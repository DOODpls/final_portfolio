...
pgrtrs.get('/search', async function(request, response){
  ...
  const blogsearch = await bloglist.find().or([{title: {$regex: searchq, $options: "i"}}, {category: {$regex: searchq, $options: "i"}}, {tags: {$regex: searchq, $options: "i"}}])
  response.render('blogsearch',{
    blogres: blogsearch,
    query: searchq,
    pginfo: pagesInfo.searchresult
  }
  )
