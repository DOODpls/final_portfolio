...
pgrtrs.get('/blogs', async function(request, response){
  const blogs = await bloglist.find().sort({"_id": -1},{"date": -1}).limit(10);
  bloglist.distinct('category', function(err, results){
...
pgrtrs.get("/get-post/:start/:limit", async function (request, response){
  const blogs = await bloglist.find().sort({"_id": -1},{"date": -1}).skip(parseInt(request.params.start)).limit(parseInt(request.params.limit))
  bloglist.distinct('category', function(err, results){
    response.send({
      pginfo: pagesInfo.index,
      blogss: blogs,
