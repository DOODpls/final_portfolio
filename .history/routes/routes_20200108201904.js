...
pgrtrs.get("/get-post/:start/:limit", async function (request, response){
  ...
  if ( url == '/search/?q='+searchq ){
    blogs = await bloglist.find().or([{title: {$regex: searchq, $options: "i"}}, {category: {$regex: searchq, $options: "i"}}, {tags: {$regex: searchq, $options: "i"}}, {blog_cont: {$regex: searchq, $options: "i"}}]).sort({"_id": -1}).skip(parseInt(request.params.start)))
  } else if (url == '/blogs'){
