...
pgrtrs.get("/get-post/:start/:limit", async function (request, response){
  ...
  const url2 = '/search/?q='+searchq;
  console.log(searchurl+"tae")
  console.log(conf+"mooo")
  if ( searchurl === url2){
    const blogs = await bloglist.find().eval(conf).sort({"date": -1,"_id": -1}).skip(parseInt(request.params.start)).limit(parseInt(request.params.limit))
  const categories = await bloglist.distinct('category');
    response.send({
      pginfo: pagesInfo.index,
      blogss: blogs,
    ...
    const blogs = await bloglist.find().sort({"date": -1,"_id": -1}).skip(parseInt(request.params.start)).limit(parseInt(request.params.limit))
  const categories = await bloglist.distinct('category');
    response.send({
      pginfo: pagesInfo.index,
      blogss: blogs,
...
pgrtrs.get('/search', async function(request, response){
  searchurl = request.originalUrl;
  searchq = request.query.q
  conf = 'or([{title: {$regex: searchq, $options: "i"}}, {category: {$regex: searchq, $options: "i"}}, {tags: {$regex: searchq, $options: "i"}}, {blog_cont: {$regex: searchq, $options: "i"}}])'
  try{
    if(searchq == ""){
      response.redirect('/search/404');
    }else{
        const blogsearch = await bloglist.find().or([{title: {$regex: searchq, $options: "i"}}, {category: {$regex: searchq, $options: "i"}}, {tags: {$regex: searchq, $options: "i"}}, {blog_cont: {$regex: searchq, $options: "i"}}]).sort({"_id": -1}).limit(2);
