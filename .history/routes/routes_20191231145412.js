  ...
  try{
    if(searchq == ""){
      response.redirect('/search/404');
        ...
        const blogs = await bloglist.find().or([{tags: request.params.items}]).sort({"_id": -1}).limit(2);
        response.render('blogsearch',{
        blogres: blogs,
        query: searchq,
        pagettl: title,
  ...
  try{
    if(searchq == ""){
      response.redirect('/search/404');
        ...
        const blogs = await bloglist.find().or([{category: request.params.items}]).sort({"_id": -1}).limit(2);
        response.render('blogsearch',{
        blogres: blogs,
        query: searchq,
        pagettl: title,
