...
pgrtrs.get('/tag', async function(request, response){
  searchq = request.query.q
  url = request.originalUrl;
  console.log(request.params.items)
  title= "Tag: "
  try{
    if(searchq == ""){
      response.redirect('/search/404');
    }else{
        const blogs = await bloglist.find().or([{tags: {$regex: request.params.items, $options: "i"}}]).sort({"_id": -1}).limit(2);
        response.render('blogsearch',{
        blogres: blogs,
        query: searchq,
        pagettl: title,
...
pgrtrs.get('/category', async function(request, response){
  searchq = request.query.q
  url = request.originalUrl;
  title= "Category: "
  try{
