...
pgrtrs.get('/blogs', async function(request, response){
  ...
  bloglist.distinct('tags', function(err, results){
    bloglist.distinct('categories', function(err, results){
  ...
  try{
    if(searchq == ""){
      response.redirect('/search/404');
        ...
        const blogsearch = await bloglist.find().or([{title: {$regex: searchq, $options: "i"}}, {category: {$regex: searchq, $options: "i"}}, {tags: {$regex: searchq, $options: "i"}}, {blog_cont: {$regex: searchq, $options: "i"}}]).sort({"_id": -1}).limit(10);
