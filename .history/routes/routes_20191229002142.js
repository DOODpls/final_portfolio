  ...
  try{
    if(searchq == ""){
      response.redirect('/search/404');
        ...
        const blogsearch = await bloglist.find().or([{title: {$regex: searchq, $options: "i"}}, {category: {$regex: searchq, $options: "i"}}, {tags: {$regex: searchq, $options: "i"}}, {blog_cont: {$regex: searchq, $options: "i"}}]).limit(10);
