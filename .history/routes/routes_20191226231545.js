...
pgrtrs.get('/search', async function(request, response){
  ...
  const blogsearch = await bloglist.find().or([{title: {$regex: searchq, $options: "i"}}, {category: {$regex: searchq, $options: "i"}}, {blog_cont: {$regex: searchq, $options: "i"}}, {blog_cont: {$regex: searchq, $options: "i"}}])
