...
function renderPosts(post){
  if (post.length > 0){
    ...
    for(var a = 0; a < post.length; a++){
      html += '<% post[a].slice().reverse().forEach(function(items){ %>';
      html += '<div class="blogcontainer">';
      html += '<a href="/blogs/<%= items.slug %>"><h2 class="blog-title"><%= items.date %>: <%= items.title %></h2></a>';
      html += '<p class="blog-content" id="blog-content"><%= items.blog_summary %></p>';
      html += '</div>';
      html += '<% }) %>';
    }
    $("#searchres-inner-div").html(html)
  }
}
