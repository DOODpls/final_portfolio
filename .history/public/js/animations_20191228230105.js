...
function renderPosts(post){
  if (post.length > 0){
    ...
    for(var a = 0; a < post.length; a++){
      ...
      html += '<a href="/blogs/ + post[a].slug + "><h2 class="blog-title"> + post[a].date + : +  post[a].title + </h2></a>';
      html += '<p class="blog-content" id="blog-content"> + post[a].blog_summary + </p>';
      html += '</div>';
    }
    document.querySelector("#searcres-inner-div").appendChild(hmtl);
  }
}
