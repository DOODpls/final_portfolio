...
function renderPosts(blogss){
  if (blogss.length > 0){
    var html = "";
    for(var a = 0; a < blogss.blogss.length; a++){
      html += '<div class="blogcontainer">';
      html += '<a href="/blogs/ + blogss.blogss[a].slug + "><h2 class="blog-title"> + blogss.blogss[a].date + : +  blogss.blogss[a].title + </h2></a>';
      html += '<p class="blog-content" id="blog-content"> + blogss.blogss[a].blog_summary + </p>';
      html += '</div>';
    }
    console.log(aaa);
    $("#blog-inner-cont").html(html)
  }
}
